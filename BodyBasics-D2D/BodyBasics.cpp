                ...
                if (pBody)
                {
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            int action = DetectDefaultGesture(rightHandState, leftHandState, jointPoints[JointType_HandRight], jointPoints[JointType_HandLeft]);
                            switch (action) 
                            {
                                case 0:
                                    DrawHand(rightHandState, jointPoints[JointType_HandRight]);
...
int CBodyBasics::DetectHand(HandState handState, const D2D1_POINT_2F& handPosition) 
{
    D2D1_ELLIPSE ellipse = D2D1::Ellipse(handPosition, c_HandSize, c_HandSize);
...
int CBodyBasics::DetectDefaultGesture(HandState handStateR, HandState handStateL, const D2D1_POINT_2F& handPositionR, const D2D1_POINT_2F& handPositionL)
{
    int leftnum = DetectHand(handStateL, handPositionL);
    int rightnum = DetectHand(handStateR, handPositionR);

    CommandExecutor executor;
    
    if (trigger_right >= trigger_time_uplimit)
    {
        trigger_right = 0;
    ...
    if (trigger_left >= trigger_time_uplimit)
    {
        trigger_left = 0;
